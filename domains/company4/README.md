# Company 4 domain (scaffold)

Placeholder child context for a future company or project.

## When ready

1. Add domain-specific assets under `rules/`, `skills/`, `commands/`, `docs/`, and optional `hooks/`.
2. Open `workspaces/company4-workspace.code-workspace` (sets `AI_EXECUTION_CONTEXT=$1`).
3. Run `.\scripts\sync-cursor.ps1` from the octo-cluster root.

See [`docs/guides/add-child-context.md`](../../docs/guides/add-child-context.md) for the full guide.
