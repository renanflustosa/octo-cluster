# OpenPolvo manual A/B pilot — ponytail-lite

One real OPE card, two arms, same ticket scope. Repo pinned at start.

## Prerequisites

- `octo-cluster` synced (`.\scripts\sync-cursor.ps1`)
- `openpolvo` clone with `develop` up to date
...
| Step | Action |
|------|--------|
| 1 | Pick ticket OPE-XXX; note repo SHA at pilot start |
...
| 4 | Score each arm: `score-diff.ps1 -RepoRoot <openpolvo> -BaseRef <pin-sha> -HeadRef HEAD -ExcludeTests` |
| 5 | Run verify gates: `go build ./...` and `go vet ./...` in `openpolvobackend` (per `repo-policies/openpolvo.yaml`) |
| 6 | If backend touched trust boundaries (paths, SQL, auth, CSV parse): run applicable `score-safety.py --task ...` on produced module |
| 7 | Write `eval/agentic/results/<YYYY-MM-DD>-openpolvo-pilot.md` |

## Results template

```markdown
# OpenPolvo pilot — YYYY-MM-DD
